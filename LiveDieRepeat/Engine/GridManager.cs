...
namespace LiveDieRepeat.Engine
{
    ...
    public class GridManager
    {
        ...
        public GridManager()
        {
            columns = (Resolution.VirtualViewport.Width / cellSize);
            rows = (int)Math.Ceiling((double)Resolution.VirtualViewport.Height / (double)cellSize);
        ...
        private List<int> GetCollidableGridIndices(ICollidable collidable)
        {
            ...
            Vector2 minCorner = new Vector2(collidable.CollisionBox.X, collidable.CollisionBox.Y);
            Vector2 maxCorner = new Vector2(collidable.CollisionBox.X + collidable.CollisionBox.Width, collidable.CollisionBox.Y + collidable.CollisionBox.Height);

            int gridIndexMinCorner = GetGridIndexForPosition(minCorner);
            int gridIndexMaxCorner = GetGridIndexForPosition(maxCorner);
            int gridIndexMinMaxCorner = GetGridIndexForPosition(new Vector2(minCorner.X, maxCorner.Y));
            int gridIndexMaxMinCorner = GetGridIndexForPosition(new Vector2(maxCorner.X, minCorner.Y));
        ...
        /// <returns></returns>
        private int GetGridIndexForPosition(Vector2 position)
        {
